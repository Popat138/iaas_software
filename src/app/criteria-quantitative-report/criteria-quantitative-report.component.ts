...
@Component({
  selector: 'app-criteria-quantitative-report',
  templateUrl: './criteria-quantitative-report.component.html',
  styleUrls: ['./criteria-quantitative-report.component.scss']
})
export class CriteriaQuantitativeReportComponent implements OnInit {
  college;
  criteria;
  crh;
  constructor(
  ...
  dataSource!:MatTableDataSource<CriteriaQuantitativeReportComponent>;
  ngOnInit(): void {
    this.fetchData();
  }
  addiqacmeetings(){
    this.fetchData();
    const dialogRef = this.dialog.open(AddCriquantitativeReportComponent ,{width: "85%",height: "86vh",panelClass: 'full-width-dialog',data : {"meetingId":2}});
   ...
   fetchData() {
    ...
    this.service.getData("/cri-quant/crh/"+user.crh.id).subscribe((res : any) => {
    ...
    viewdetails(element: any){const dialogRef = this.dialog.open(ViewCriteriaQuantitativeComponent ,{width: "85%",height: "86vh",panelClass: 'full-width-dialog', data: element});
    dialogRef.afterClosed().subscribe((result : any) => {
       this.fetchData();
    });
    ...
    edit(row){
      const dialogRef = this.dialog.open(EditCriquantitativeReportComponent,{width: "70%",height: "86vh",panelClass: 'full-width-dialog',data: row});
      dialogRef.afterClosed().subscribe((result : any) => {
        this.fetchData();
     });
    }
    ...
    delete(row){
      console.log(row);
        ...
        this.service.deleteData('/cri-quant/'+row['id']).subscribe(response => {
          location.reload();
      },err => {
        // console.log(err);
        if(err.status == 409){
