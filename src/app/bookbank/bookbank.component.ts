...
export class BookbankComponent implements OnInit {
  ...
  dataSource!:MatTableDataSource<BookbankComponent>;
  ...
  addpgcourses(){
    const dialogRef = this.dialog.open(AddBookBankComponent ,{width: "80%",height: "86vh",panelClass: 'full-width-dialog'});
  ...
  delete(row){
    // console.log(row);
      ...
      this.service.deleteData('/book-bank/'+row['id']).subscribe(response => {
        location.reload();
    },err => {
      // console.log(err);
      if(err.status == 409){
