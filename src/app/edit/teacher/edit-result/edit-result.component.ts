...
export class EditResultComponent implements OnInit {
  ...
  getData() {
    ...
    this.Form.get("year").setValue(this.data.academicDetail?.academicYear);
    ...
    this.http.get(`${this.service.BASE_URL}/resources/uploads/${this.data.resultDoc}`,{responseType: 'blob'}).subscribe(data => {
      this.report = data;
      this.report.name = this.data.resultDoc;
  });
  }
  ...
  submitForm(){
    ...
    this.service.putData(`/result`, data).subscribe((res: any) => {
      console.log(res);
