...
export class EditProgramDetailsComponent implements OnInit {
  ...
  remove(subject: any, i:number): void {
    ...
    constructor(
      ...
      public dialogRef: MatDialogRef<EditProgramDetailsComponent>,
  ...
  getData() {
    ...
    this.service.getData("/department/program/" + this.data.program.programId).subscribe((department: any) => {
      // console.log(department);
      this.Form.get("department").setValue(this.data.department);
      this.Form.get("prog_name").setValue(this.data.program.programId);
      this.Form.get("class").setValue(this.data.programClass);
  ...
  fetchDepartment() {
    ...
    submitForm(){
      ...
      for(let i = 0; i< this.division.length; i++){
        ...
        divsionList.push({
          id: this.division.at(i).get("id").value,
      ...
      let programDetails = {
        id: this.data.id,
        programClass: this.Form.get("class").value,
        intakeCapacity: this.Form.get("intake").value,
        certificate: uploadFilename,
        divisions: divsionList,
        program: {
          programId: this.Form.get("prog_name").value
        }
      }

      this.service.putData("/program-detail", programDetails).subscribe((res:any) => {
