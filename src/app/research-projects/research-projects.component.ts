...
export class ResearchProjectsComponent implements OnInit {
  ...
  displayedColumns: string[] = ['academic_year','award_year','level','dept','title_of_book','authors','year_award','publisher','edition','agency','upload_cert','edit','delete'];
  constructor(
    private dialog: MatDialog,
    public service: ServiceService
  ) { }
  dataSource!:MatTableDataSource<ResearchProjectsComponent>;
  ...
  edit(row){
    const dialogRef = this.dialog.open(EditResrearchProjectDetailsComponent,{width: "70%",height: "86vh",panelClass: 'full-width-dialog',data: row});
    dialogRef.afterClosed().subscribe((result : any) => {
      this.fetchData();
   });
  }
  ...
  fetchData() {
    this.service.getUserWithUserId().subscribe((user: any) => {
      ...
      this.service.getData("/research-project/teacher/" + user.teacher.teacherId).subscribe((res: any) => {
        console.log(res);
